...
const mapBy = R.curry((ops, predicate, transformation, nodes) => {
    ops = { ...defaultOps, ...ops };
    ...
    const result = operators_map.map(ops, R.when(R.both(predicate, R.pipe(counter.getCount, R.lt(R.__, maxApplyTimes))), R.pipe(transformation, R.tap(() => counter.up()))), nodes);
