...
describe("find", () => {
  test("최대 filter 적용 횟수가 0인 경우, error를 throw한다.", () => {
    ...
    expect(() => findBy(ops, () => true, originalNodes)).toThrow();
  });
  ...
  test("최대 find 적용 횟수가 1인 경우, 최대 1번만 특정 조건을 만족하는 node에 적용된다. ", () => {
    ...
    expect(findBy(ops, predicate, originalNodes)).toEqual(expectedNode);
  });
  ...
  test("다중 find ... 생략 ", () => {
    ...
    expect(findBy(ops, predicate, originalNodes)).toEqual(
  ...
  test("최소 filter 적용 횟수가 100인 경우, 실제 find 적용횟수가 미달일 시, error를 throw한다. ", () => {
    ...
    expect(() => findBy(ops, predicate, originalNodes)).toThrow();
  });
});
